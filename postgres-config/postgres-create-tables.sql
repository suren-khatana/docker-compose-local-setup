...
COMMENT ON COLUMN delegations.owner IS 'Moment when delegations record is created, as measured in number of seconds since epoch';
...
CREATE TABLE accounts (
  account_id  VARCHAR(64)   PRIMARY KEY NOT NULL DEFAULT uuid_generate_v1(),
...
COMMENT ON COLUMN dynamically_registered_clients.client_secret IS 'The hash of this client''s secret';
COMMENT ON COLUMN dynamically_registered_clients.instance_of_client IS 'The client ID on which this instance is based, or NULL if this is a non-templatized client';
